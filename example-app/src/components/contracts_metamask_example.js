...
async function setWeb3Connection(network, user) {
  ...
  if (window.ethereum) {
    ...
    contractsMetamask = {
      public: new web3metamask.eth.Contract(artifactPublic.abi, contractAddressPublic),
      east:   new web3metamask.eth.Contract(artifactEast.abi, contractAddressEast),
    }
  }
...
async function transfer(network, contractType, sender, recipient, amount) {
  ...
  try {
    if (contractType === 'public') {
      let contractMetamask = contractsMetamask['public']
      receipt = await contractMetamask.methods
